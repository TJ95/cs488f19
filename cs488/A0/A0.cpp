...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// rotation slider using m_shape_rotation
		ImGui::SliderFloat("Rotation", &m_shape_rotation, 0.0f, glm::radians(360.0f));
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			m_shape_size = m_shape_size*1.06;
		...
		if (key == 'R') {
			cout << "R key pressed, resetting shape..." << endl;
